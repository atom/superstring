...
static Nan::Maybe<Point> PointFromJS(Nan::MaybeLocal<Object> maybe_object) {
  ...
  if (!maybe_object.ToLocal(&object)) {
    Nan::ThrowTypeError(
        "Expected an object with 'row' and 'column' properties.");
    return Nan::Nothing<Point>();
  }

  Nan::MaybeLocal<Integer> maybe_row =
      Nan::To<Integer>(object->Get(Nan::New(row_string)));
  ...
  Nan::MaybeLocal<Integer> maybe_column =
      Nan::To<Integer>(object->Get(Nan::New(column_string)));
...
public:
  static void Init() {
    ...
    constructor_template->SetClassName(
        Nan::New<String>("Point").ToLocalChecked());
    constructor_template->InstanceTemplate()->SetInternalFieldCount(1);
    Nan::SetAccessor(constructor_template->InstanceTemplate(),
                     Nan::New(row_string), GetRow);
    Nan::SetAccessor(constructor_template->InstanceTemplate(),
                     Nan::New(column_string), GetColumn);
    constructor.Reset(constructor_template->GetFunction());
  }

  static Local<Value> FromPoint(Point point) {
    Local<Object> result;
    if (Nan::New(constructor)
...
public:
  static void Init() {
    Local<FunctionTemplate> constructor_template =
        Nan::New<FunctionTemplate>(New);
    constructor_template->SetClassName(
        Nan::New<String>("Hunk").ToLocalChecked());
    constructor_template->InstanceTemplate()->SetInternalFieldCount(1);
    const auto &instance_template = constructor_template->InstanceTemplate();
    Nan::SetAccessor(instance_template, Nan::New("oldStart").ToLocalChecked(),
                     GetOldStart);
    Nan::SetAccessor(instance_template, Nan::New("newStart").ToLocalChecked(),
                     GetNewStart);
    Nan::SetAccessor(instance_template, Nan::New("oldEnd").ToLocalChecked(),
                     GetOldEnd);
    Nan::SetAccessor(instance_template, Nan::New("newEnd").ToLocalChecked(),
                     GetNewEnd);

    // Non-enumerable legacy properties for backward compatibility
    Nan::SetAccessor(instance_template, Nan::New("start").ToLocalChecked(),
                     GetNewStart, nullptr, Handle<Value>(),
                     AccessControl::DEFAULT, PropertyAttribute::DontEnum);
    Nan::SetAccessor(instance_template, Nan::New("oldExtent").ToLocalChecked(),
                     GetOldExtent, nullptr, Handle<Value>(),
                     AccessControl::DEFAULT, PropertyAttribute::DontEnum);
    Nan::SetAccessor(instance_template, Nan::New("newExtent").ToLocalChecked(),
                     GetNewExtent, nullptr, Handle<Value>(),
                     AccessControl::DEFAULT, PropertyAttribute::DontEnum);

    const auto &prototype_template = constructor_template->PrototypeTemplate();
    prototype_template->Set(Nan::New<String>("toString").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(ToString));
    constructor.Reset(constructor_template->GetFunction());
  }
...
private:
  ...
  static void GetOldStart(v8::Local<v8::String> property,
                          const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(PointWrapper::FromPoint(hunk.old_start));
  }

  static void GetNewStart(v8::Local<v8::String> property,
                          const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(PointWrapper::FromPoint(hunk.new_start));
  }

  static void GetOldEnd(v8::Local<v8::String> property,
                        const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(PointWrapper::FromPoint(hunk.old_end));
  }

  static void GetNewEnd(v8::Local<v8::String> property,
                        const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(PointWrapper::FromPoint(hunk.new_end));
  }

  static void GetOldExtent(v8::Local<v8::String> property,
                           const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(
        PointWrapper::FromPoint(hunk.old_end.Traversal(hunk.old_start)));
  }

  static void GetNewExtent(v8::Local<v8::String> property,
                           const Nan::PropertyCallbackInfo<v8::Value> &info) {
    Hunk &hunk = Nan::ObjectWrap::Unwrap<HunkWrapper>(info.This())->hunk;
    info.GetReturnValue().Set(
        PointWrapper::FromPoint(hunk.new_end.Traversal(hunk.new_start)));
  }
...
public:
  static void Init(Local<Object> exports, Local<Object> module) {
    Local<FunctionTemplate> constructor_template_local =
        Nan::New<FunctionTemplate>(New);
    constructor_template_local->SetClassName(
        Nan::New<String>("Patch").ToLocalChecked());
    constructor_template_local->Set(Nan::New("deserialize").ToLocalChecked(),
                                    Nan::New<FunctionTemplate>(Deserialize));
    constructor_template_local->Set(Nan::New("compose").ToLocalChecked(),
                                    Nan::New<FunctionTemplate>(Compose));
    constructor_template_local->InstanceTemplate()->SetInternalFieldCount(1);
    const auto &prototype_template =
        constructor_template_local->PrototypeTemplate();
    prototype_template->Set(Nan::New("splice").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(Splice));
    prototype_template->Set(Nan::New("spliceOld").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(SpliceOld));
    prototype_template->Set(Nan::New("invert").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(Invert));
    prototype_template->Set(Nan::New("getHunks").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(GetHunks));
    ...
    prototype_template->Set(Nan::New("serialize").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(Serialize));
    prototype_template->Set(Nan::New("printDotGraph").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(PrintDotGraph));
    prototype_template->Set(Nan::New("rebalance").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(Rebalance));
    prototype_template->Set(Nan::New("getHunkCount").ToLocalChecked(),
                            Nan::New<FunctionTemplate>(GetHunkCount));
...
private:
  ...
  static void New(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    if (info.Length() > 0 && Nan::To<Object>(info[0]).ToLocal(&options)) {
      ...
      if (Nan::To<Boolean>(
              options->Get(Nan::New("mergeAdjacentHunks").ToLocalChecked()))
  ...
  static void Splice(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    if (start.IsJust() && deletion_extent.IsJust() &&
        insertion_extent.IsJust()) {
      ...
      if (!patch.Splice(start.FromJust(), deletion_extent.FromJust(),
                        insertion_extent.FromJust(), move(deleted_text),
                        move(inserted_text))) {
  ...
  static void SpliceOld(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    if (start.IsJust() && deletion_extent.IsJust() &&
        insertion_extent.IsJust()) {
      if (!patch.SpliceOld(start.FromJust(), deletion_extent.FromJust(),
                           insertion_extent.FromJust())) {
  ...
  static void GetHunksInOldRange(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    if (start.IsJust() && end.IsJust()) {
      ...
      size_t i = 0;
      for (Hunk hunk :
           patch.GetHunksInOldRange(start.FromJust(), end.FromJust())) {
        js_result->Set(i++, HunkWrapper::FromHunk(hunk));
      }
  ...
  static void GetHunksInNewRange(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    if (start.IsJust() && end.IsJust()) {
      ...
      size_t i = 0;
      for (Hunk hunk :
           patch.GetHunksInNewRange(start.FromJust(), end.FromJust())) {
        js_result->Set(i++, HunkWrapper::FromHunk(hunk));
      }
  ...
  static void Serialize(const Nan::FunctionCallbackInfo<Value> &info) {
    ...
    Local<Object> result;
    auto maybe_result =
        Nan::CopyBuffer(reinterpret_cast<char *>(serialization_vector.data()),
    ...
    if (Nan::NewInstance(Nan::New(constructor)).ToLocal(&result)) {
      ...
      for (uint32_t i = 0, n = js_patches->Length(); i < n; i++) {
        ...
        if (!Nan::New(constructor_template)->HasInstance(js_patch)) {
          Nan::ThrowTypeError(
              "Patch.compose must be called with an array of patches");
          return;
        }
...
void Init(Local<Object> exports, Local<Object> module) {
  ...
  column_string.Reset(
      Nan::Persistent<String>(Nan::New("column").ToLocalChecked()));
  new_text_string.Reset(
      Nan::Persistent<String>(Nan::New("newText").ToLocalChecked()));
  old_text_string.Reset(
      Nan::Persistent<String>(Nan::New("oldText").ToLocalChecked()));
